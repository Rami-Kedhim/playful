...
serve(async (req) => {
  ...
  try {
    ...
    if (type === "image") {
      return await handleImageGeneration(requestData, userProfile, OPENAI_API_KEY, supabaseAdmin, corsHeaders);
    } else if (type === "message") {
      return await handleMessageGeneration(requestData, userProfile, OPENAI_API_KEY, supabaseAdmin, corsHeaders);
    } else {
...
async function handleImageGeneration(
  ...
  const { prompt, user_id, ai_profile_id, size = "1024x1024", style = "natural" } = requestData;
...
async function handleMessageGeneration(
  ...
  if (!conversation_id || !user_message || !ai_profile_id) {
