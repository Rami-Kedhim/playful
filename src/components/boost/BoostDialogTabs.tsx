...
import BoostEligibilityCheck from '../boost/BoostEligibilityCheck';
...
const BoostDialogTabs: React.FC<BoostDialogTabsProps> = ({
  ...
  const [selectedPackage, setSelectedPackage] = React.useState<string | null>(
    packages && packages.length > 0 ? packages[0].id : null
  );
    ...
    return (
      <div className="mb-4">
        <BoostEligibilityCheck 
          ...
          onClose={() => {}} // Empty function to satisfy prop requirement
        />
      </div>
    );
  };
